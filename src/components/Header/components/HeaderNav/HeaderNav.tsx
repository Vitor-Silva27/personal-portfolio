...
export const HeaderNav = ({about, contact,projects, lang}: IHeaderNavProps) => {
  ...
  return (
    <div>
        ...
        <nav className={"sm:flex gap-10 list-none font-display sm:text-xl tracking-widest text-zinc-600 sm:items-center " + (mobileNavOpen ? "flex flex-col fixed top-0 right-0 bottom-0 left-0 bg-slate-100 z-50 p-8 text-3xl h-screen pt-28" : " hidden")}>
            <Link onClick={() => setMobileNavOpen()} className="transition-all hover:-translate-y-1 hover:text-zinc-950" href="#about"><li>{about}</li></Link>
            <Link onClick={() => setMobileNavOpen()} className="transition-all hover:-translate-y-1 hover:text-zinc-950" href="#projects"><li>{projects}</li></Link>
            <Link onClick={() => setMobileNavOpen()} className="transition-all hover:-translate-y-1 hover:text-zinc-950" href="#contact"><li>{contact}</li></Link>
